...
# 永続化
cat << 'EOF' > /etc/sysctl.d/99-mion.conf
...
sequenceDiagram
    ...
    Note over C: VerifyPeerCertificate:<br/>pubkey → SHA-256 → peer_id<br/>peer_id ∈ KnownPeers ?
    Note over P: VerifyPeerCertificate:<br/>pubkey → SHA-256 → peer_id<br/>peer_id ∈ KnownPeers ?
